...
def parse(doc=None, url=None):
    ...
    if doc is None:
        try:
            ...
            response = web.get(url, verify=False)
        doc = response.text
    data = mf2py.parse(doc, url)
    data.pop("debug")
    return data
...
def representative_card(parsed, source_url):
    ...
    if match := _check_uid_and_url_match_source_url(source_url, cards):
        return match
    if match := _check_url_matches_rel_me(cards):
        return match
    if match := _check_url_matches_source_url(source_url, cards):
        return match


def _get_normal(card, prop):
    """Return a list of normalized URLs for an card's prop (uid/url)."""
    urls = []
    for url in card["properties"].get(prop, []):
        try:
            urls.append(uri.parse(url).minimized)
        except ValueError:
            pass
    return urls


def _check_uid_and_url_match_source_url(source_url, cards):  # FIXME same as below?
    for card in cards:
        if source_url in _get_normal(card, "uid") and source_url in _get_normal(
            card, "url"
        ):
            return card["properties"]


def _check_url_matches_rel_me(cards):
    for card in cards:
        rel_mes = set()
        ...
        if any(url in rel_mes for url in _get_normal(card, "url")):
            return card["properties"]


def _check_url_matches_source_url(source_url, cards):  # FIXME same as above?
    found = []
    count = 0
    for card in cards:
        # if source_url in card['properties'].get('url', []):
        for card_url in _get_normal(card, "url"):
...
util.representative_hcard = representative_card
